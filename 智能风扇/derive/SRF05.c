...
{  
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);
        ...
        //IO初始化
    GPIO_InitStructure.GPIO_Pin =SRF05_TRIG_GPIOB;       //发送电平引脚
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;//推挽输出
    GPIO_Init(GPIOB, &GPIO_InitStructure);
    GPIO_ResetBits(GPIOB,SRF05_TRIG_GPIOB);
     
    GPIO_InitStructure.GPIO_Pin =SRF05_ECHO_GPIOB;     //返回电平引脚
...
float SRF05GetLength(void )
{
        ...
        while(i!=5)
        {
					...
					GPIO_WriteBit(GPIOB,SRF05_TRIG_GPIOB,Bit_SET);//发送口高电平输出

					delay_us(20);
					GPIO_WriteBit(GPIOB,SRF05_TRIG_GPIOB,Bit_RESET);
					while(GPIO_ReadInputDataBit(GPIOB,SRF05_ECHO_GPIOB)==0);      //等待接收口高电平输出
