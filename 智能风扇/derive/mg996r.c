...
void pwm_config(void)
{
   ...
   //设置该引脚为复用输出功能,输出TIM1 CH1的PWM脉冲波形
	...
	TIM_TimeBaseStructure.TIM_Period = 2999; //设置在下一个更新事件装入活动的自动重装载寄存器周期的值	 80K
...
//jiaodutemp=499+10.811*jiaodu
	if(n)
		{
			angle_y=angle_y+x;
			if(angle_y>160)angle_y=160;//舵机y轴限制在160度
	...
	{
			angle_x=angle_x+x;
			if(angle_x>180)angle_x=180;//舵机不能超过180度
...
//jiaodutemp=499+10.811*jiaodu
		if(n)
		{
			if(angle_y<x)angle_y=0;//舵机不能超过0度
		...
		{
			if(angle_x<x)angle_x=0;//舵机不能超过0度
